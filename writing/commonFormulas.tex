...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=3cm,
  ...
  \node[state] 		   (AA)  [below of=A]  {\small $(\pi_1,q_2)$};
  \node[state]         (BB) [ right of=AA] {\small $(\pi_2,q_2)$};
  \node[state]         (CC) [right of=BB] {\small $(\pi_3,q_2)$};
  \node[state]         (DD) [right of=CC] {\small $(\pi_4,q_2)$};
  \node[state]         (EE) [right of=DD] {\small $(\pi_5,q_2)$};
  
  \node[state,accepting] 		   (AAA)  [below of=AA]  {\small $(\pi_1,q_3)$};
  \node[state,accepting]         (BBB) [ right of=AAA] {\small $(\pi_2,q_3)$};
  \node[state,accepting]         (CCC) [right of=BBB] {\small $(\pi_3,q_3)$};
  \node[state,accepting]         (DDD) [right of=CCC] {\small $(\pi_4,q_3)$};
  \node[state,accepting]         (EEE) [right of=DDD] {\small $(\pi_5,q_3)$};
...
Recurrence is coverage over and over again, and can be expressed as $\ssquare(\diamond \pi_1 \land \diamond \pi_2 \land \dots \land \diamond \pi_n)$. This example is interesting for two reasons: it is prone to B\"{u}chi automata that are not tight \cite{schuppan05}, and an accepting path for it does cannot stay in one state (in contrast to the other formulas, in which all accepting states have self loops). We first look at the tightness.

\subsubsection*{Tightness}
A tight B\"uchi automaton is one that accepts the minimum prefix and the minimum suffix. We begin showing that the automaton produced for these formulas by \cite{ltlbuchiwebsite} is not tight. We consider the formula $\ssquare(\diamond \pi_1 \land \diamond \pi_2 \land \diamond \pi_3)$. The B\"{u}chi automaton corresponding to this formula, as calculated by \cite{gastin01} is given in figure \ref{fig:gasBuchiRec}
...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=5cm,
  ...
  \path (A) edge         node {$\pi_{1}$}   (B)
        (B) edge       node {$\pi_{2}$}     (D)
        (A) edge       node [near end] {$\pi_{2}$}  (C)
        (C) edge     node {$\pi_{1}$}       (D)
        (A) edge [loop above] node {$\neg \pi_1 \wedge \neg \pi_2$} (A)
        (B) edge [loop above] node {$\neg \pi_2$} (B)
        (C) edge [loop below] node {$\neg \pi_1$} (C)
        (D) edge [bend left] node {$\pi_2$} (C)
        (D) edge [bend right] node [near end] {$\pi_1$} (B)
        (D) edge node{$\neg \pi_1 \wedge \neg \pi_2$} (A);
\end{tikzpicture}
...
The algorithms produced the same path. 
\begin{lstlisting}
the prefix of plan **states**:
